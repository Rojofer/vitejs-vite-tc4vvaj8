                  ...
                  <motion.div key={tpl.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm group">
                    <div className={`flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 ${expandedTemplates.includes(tpl.id) ? 'border-b border-slate-100 pb-4 mb-4' : ''}`}>
                      ...
                      <div className="flex gap-2 shrink-0 flex-wrap">
                        ...
                        <button onClick={() => { const nuevas = [...getPlantillas()]; nuevas[index].isMultiple = !nuevas[index].isMultiple; setLocalConfig({...localConfig, plantillasDinamicas: nuevas}); }} className={`px-3 py-1.5 rounded text-[9px] font-black uppercase tracking-widest transition-all border ${tpl.isMultiple ? 'bg-emerald-500 text-white border-emerald-600' : 'bg-white text-slate-400 border-slate-200 hover:bg-emerald-50 hover:text-emerald-500'}`} title="Usar por defecto cuando se reclaman múltiples insumos de una misma OC"> Default Multiple OC </button>
