...
const VistaNotificaciones = ({
  ...
  return (
    <div className="p-4 md:p-6 h-full w-full mx-auto max-w-7xl">
          ...
          {currentUser.rol === 'owner' ? (
            <>
              <button 
                ...
                className={`pb-2 font-black text-xs uppercase tracking-widest transition-all border-b-2 ${tabActiva === 'tickets' ? 'border-sky-500 text-sky-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
              >
                Tickets en Curso
              </button>
              <button 
                onClick={() => setTabActiva('avisos')} 
                className={`pb-2 font-black text-xs uppercase tracking-widest transition-all border-b-2 ${tabActiva === 'avisos' ? 'border-yellow-500 text-yellow-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
              >
                Historial Avisos
              </button>
            </>
          ) : (
            <span className="pb-2 font-black text-xs uppercase tracking-widest border-b-2 border-yellow-500 text-yellow-600">
              Bandeja de Avisos
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold outline-none focus:border-sky-500 transition-all shadow-sm" 
            />
          </div>
          ...
          {currentUser.rol === 'owner' && !modoRedaccion && tabActiva === 'avisos' && (
            <button 
              ...
              className="bg-yellow-500 hover:bg-yellow-600 text-slate-900 px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-sm transition-all whitespace-nowrap"
            >
              <Megaphone size={16} /> Emitir
            </button>
          )}
        </div>
      </div>
        ...
        {notificacionesFiltradas.length > 0 ? (
            ...
            {notificacionesFiltradas.map((noti) => {
              ...
              return (
                <div 
                  ...
                  className={`p-5 flex gap-4 transition-colors cursor-pointer hover:bg-slate-50 ${estaLeido ? 'opacity-60 bg-white' : esTicket ? 'bg-sky-50/30' : 'bg-yellow-50/30'}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center justify-between mt-3 gap-2">
                      ...
                      {currentUser.rol === 'owner' && noti.leidoPor?.length > 0 && (
                        <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest bg-emerald-50 px-2 py-1 rounded border border-emerald-100">
          ...
          <div className="py-20 text-center flex flex-col items-center justify-center">
            <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4 border border-slate-100">
              {busqueda ? <Search size={24} className="text-slate-300" /> : <Bell size={24} className="text-slate-300" />}
            </div>
            <h3 className="text-lg font-bold text-slate-700 mb-1">{busqueda ? 'Sin resultados' : 'Bandeja al día'}</h3>
            <p className="text-sm text-slate-400 font-medium">{busqueda ? 'No se encontraron avisos con esa palabra.' : 'No tenés notificaciones pendientes.'}</p>
          </div>
        )}
      </div>
