...
xl:flex-row items-start xl:items-center justify-between mb-8 gap-6">
...
const sanos = Math.max(0, totalFavs - quiebres - riesgos);
          const score = totalFavs > 0 ?
Math.round((sanos / totalFavs) * 100) : 100;
...
bgClass = "bg-red-50"; borderClass = "border-red-200"; textClass = "text-red-700"; msj = "Riesgo de quiebres inminentes.";
  ...
  <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col justify-center shadow-sm min-w-[320px] hover:shadow-md transition-shadow shrink-0">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border ${tieneFantasmas ? 'bg-red-50 text-red-600 border-red-200 animate-pulse' : 
'bg-slate-50 text-slate-500 border-slate-100 shadow-sm'}`}>
           ...
           <div style={{ width: `${totalProductos > 0 ?
Math.round((totalFavs / totalProductos) * 100) : 0}%` }} className="h-full bg-orange-400 transition-all duration-500"></div>
                  </div>
    ...
    <div className="flex items-center gap-1.5">
              ...
              <div className={`w-full max-w-3xl rounded-2xl border ${borderClass} ${bgClass} px-5 
py-4 shadow-sm flex items-center gap-6 relative overflow-hidden`}>
           ...
           <div>
                      <h3 className={`font-black uppercase tracking-tight text-[13px] leading-none ${textClass}`}>{currentUser.rol === 'owner' ?
'Salud Operativa Global - FAVORITOS' : 'Salud Operativa del Inventario -SOLO DE FAVORITOS'}</h3>
     ...
     <span className={`text-2xl font-black leading-none tracking-tighter ${textClass}`}>{score}%</span>
           ...
           <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-sm bg-red-500 shadow-sm ${quiebres > 0 ?
'animate-pulse' : ''}`}></div>
...
(
    ...
    <TablaInsumos 
            ...
            toggleVisibilidadPlanta={toggleVisibilidadPlanta} 
              ...
              canEditFav={currentUser.rol === 'owner' ||
currentUser.editorFavoritos} 
            ...
            <motion.div key="alerta" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                  <div className="flex items-center gap-4">
                    <button onClick={() => { setFiltroAlerta(null); setFiltroVistaLista('todos'); }} className="flex items-center gap-2 text-slate-500 font-bold text-xs uppercase 
bg-white px-3 py-1.5 rounded-lg border hover:text-orange-500 shadow-sm transition-colors">
              ...
              <button onClick={() => setFiltroVistaLista('todos')} className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${filtroVistaLista === 'todos' ?
'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Todos</button>
                        <button onClick={() => setFiltroVistaLista('favoritos')} className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-1 ${filtroVistaLista === 'favoritos' ?
'bg-white text-orange-500 shadow-sm' : 'text-slate-400 hover:text-orange-400'}`}>★ Favs</button>
                        <button onClick={() => setFiltroVistaLista('nofavoritos')} className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-1 ${filtroVistaLista === 'nofavoritos' ?
'bg-white text-slate-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>☆ No Favs</button>
                      </div>
                    )}
...
(
 ...
 const misInsumosDashboard = currentUser.rol === 'owner' ? insumos : insumos.filter(i => i.owner?.toUpperCase().trim() === currentUser.aliasMatch);
                ...
                const kpiQuiebres = misInsumosDashboard.filter(i 
=> i.favorito && i.supervivencia <= uCritico);
       ...
       <span className="text-[10px] sm:text-[11px] font-black text-orange-500 uppercase tracking-widest leading-tight">En Riesgo</span>
           ...
           <div className="p-2 bg-rose-50 rounded-xl"><Clock size={16} className="text-rose-500"/></div>
               ...
               <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">De {total}</span>
                    ...
                    <div onClick={() => setFiltroAlerta('mis_favoritos')} className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm cursor-pointer hover:shadow-lg hover:-translate-y-1 hover:border-yellow-200 transition-all duration-300 
flex flex-col justify-between">
         ...
         ownersUnicos.forEach(op => {
                ...
                if (insOp.length > 0) {
                  ...
                  const 
rOp = insOp.filter(i => i.supervivencia <= uUrgencia && i.supervivencia > uCritico).length;
...
if (operariosStats.length === 0) return null;
      ...
      <div 
                          key={op.nombre} 
                          onClick={() => {
                            // Validamos que exista la función antes de llamarla para 
no romper la app
...
'cursor-pointer hover:-translate-y-1 hover:border-orange-300 ring-2 ring-transparent hover:ring-orange-100' : ''}`}
                        >
                          <div className="flex justify-between items-center">
                            <div className={`flex items-center gap-1.5 px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border 
${cStyle.bg} ${cStyle.text} ${cStyle.border} truncate max-w-[65%]`}>
                              <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${cStyle.dot}`}></span>
                              <span className="truncate">{op.nombre}</span>
                            </div>
  
                          <span className={`text-sm font-black leading-none tracking-tight ${op.score < 80 ?
'text-red-600' : op.score < 100 ? 'text-amber-600' : 'text-emerald-600'}`}>
                              {op.score}%
                            </span>
                          </div>
...
(
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => setFiltroRiesgoGrupo(!filtroRiesgoGrupo)} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-all 
border shadow-sm ${filtroRiesgoGrupo ? 'bg-red-500 text-white border-red-600' : 'bg-white text-red-500 border-red-200 hover:bg-red-50'}`}>
                  <AlertTriangle size={12} /> {filtroRiesgoGrupo ?
'Mostrando Riesgos' : 'Filtrar Riesgos'}
                </button>
              </div>
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 w-full">
           ...
           gruposAMostrar = grupos.filter(g => misGrupos.includes(g));
                ...
                if (filtroRiesgoGrupo) {
                  const uUrgencia = config?.umbralUrgencia !== undefined ?
config.umbralUrgencia : 15;
                ...
                return gruposAMostrar.map(grupo => { 
                  const itemsDelGrupo = currentUser.rol === 'owner' 
                    ...
                    : insumos.filter(i => (i.grupo || 'SIN CLASIFICAR') === 
grupo && i.owner?.toUpperCase().trim() === currentUser.aliasMatch);
     ...
     const favsDelGrupo = itemsDelGrupo.filter(i => i.favorito);
                  ...
                  const 
rG = favsDelGrupo.filter(i => i.supervivencia <= uUrgencia && i.supervivencia > uCritico).length;
...
return (
           ...
           <div className="flex justify-between items-center mb-1">
                         ...
                         <span className={`text-[9px] font-black px-2.5 py-1 rounded shadow-sm transition-colors border ${scoreG < 80 ? 'bg-red-50 text-red-700 border-red-200' : scoreG < 100 ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 
border-emerald-100'}`}>
                           {scoreG}% SALUD
                         </span>
                        </div>
...
return (
                      ...
                      {resp}
                          ...
                          <div className="flex gap-1.5 shrink-0 items-center flex-col">
                            {qG > 0 && <span className="px-2 py-1 rounded-md text-[8px] font-black uppercase bg-red-50 text-red-600 border border-red-200 
shadow-sm animate-pulse w-full text-center">{qG} QUIEBRE</span>}
  ...
  <span className="text-xl font-black text-slate-800 leading-none">{itemsDelGrupo.length}</span>
     ...
     <span className="text-[9px] font-bold text-slate-400 uppercase">Favoritos</span>
                            <span className={`text-xl font-black leading-none ${scoreG < 100 ?
'text-orange-600' : 'text-emerald-600'}`}>{favsDelGrupo.length}</span>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          (() => {
            const datosFiltradosGrupo = insumos
              ...
              .filter(i => currentUser.rol === 'owner' ? 
true : i.owner?.toUpperCase().trim() === currentUser.aliasMatch)
              .filter(i => filtroVistaLista === 'todos' ? true : filtroVistaLista === 'favoritos' ? i.favorito : !i.favorito);
